...
#' @param prefcode jis code for prefecture and city identifical number. If prefecture, must be from 1 to 47.
#' @param dest If TRUE, to unzip downloaded files.
#' @inheritParams utils::unzip
#' @export
download_stat_map <- function(prefcode, exdir = ".", dest = TRUE) {
  prefcode <-
    stringr::str_pad(prefcode, width = 2, pad = "0")
  rlang::arg_match(prefcode, values = stringr::str_pad(seq.int(47), width = 2, pad = "0"))
  x <-
    glue::glue(
      "https://www.e-stat.go.jp/gis/statmap-search/data?dlserveyId=A002005212015&code={prefcode}&coordSys=1&format=shape&downloadType=5"
    )
  qry <-
